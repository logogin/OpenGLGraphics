...
#define NUM_PARTICLES    10          /* Number of particles  */
...
if(up_view)	gluLookAt(	0, 0., 0, 0., -10., 0.,
							-cos(look_at_r), 0., sin(look_at_r));
 else	gluLookAt(	0.*cos(look_at_r), 0., -10.*sin(look_at_r), 
			0., 0., 0., 0., 1., 0.);
...
if(key =='J' || key =='j')//work with menu
	{ angle1[axis1] += 5.0;
        if( angle1[axis1] > 360.0 ) angle1[axis1] -= 360.0;
	}
if(key== 'H' || key == 'h')
	{ angle1[axis1] -= 5.0;
        if( angle1[axis1] < 0.0 ) angle1[axis1] += 360.0;
	}

if(key =='L' || key== 'l')
	{ 
        if( angle1[axis] < 0.0 ) angle1[axis] += 5.0;
	}
if(key== 'K' || key == 'k')
	{ angle1[axis] -= 5.0;
        if( angle1[axis] < -135.0 ) angle1[axis] += 5.0;
	}
	//////////////////Right hend/////////////////////////
...
if(key =='W'|| key =='w')
	{ 
        if( angle1[axis2] < 25.0 ) angle1[axis2] += 5.0;
	}
if(key== 'E'||key== 'e' )
	{ angle1[axis2] -= 5.0;
        if( angle1[axis2] < -125.0 ) angle1[axis2] += 5.0;
	}

if(key =='R' || key =='r')
	{ angle1[axis3] -= 5.0;
        if( angle1[axis3] <-20.0 ) angle1[axis3] += 5.0;
	}
if(key== 't' || key == 't')
	{ 
        if( angle1[axis3] < 90.0 ) angle1[axis3] += 5.0;
	}

/////////////////[ Right leg ]////////////////////////////////
if(key =='P' || key== 'p')
	{ 
        if( angle1[axis4] < 25.0 ) angle1[axis4] += 5.0;
	}
if(key== 'O' || key == 'o')
	{ angle1[axis4] -= 5.0;
        if( angle1[axis4] < -125.0 ) angle1[axis4] += 5.0;
	}

if(key =='I' || key =='i')
	{ angle1[axis5] -= 5.0;
        if( angle1[axis5] <-20.0 ) angle1[axis5] += 5.0;
	}
if(key== 'U' || key == 'u')
	{ 
        if( angle1[axis5] < 90.0 ) angle1[axis5] += 5.0;
	}
