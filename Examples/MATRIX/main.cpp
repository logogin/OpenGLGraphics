...
/*	PIXELFORMATDESCRIPTOR pfd_new;
	DescribePixelFormat (*hDC, iFormat, sizeof(PIXELFORMATDESCRIPTOR),
															&pfd_new);
	...
	{
		// software
		MessageBox(NULL,"software","Detecting hardware acceleration",
																MB_OK);
	}
	else if (generic_format && generic_accelerated)
	{
		// hardware - MCD
		MessageBox(NULL,"hardware - MCD","Detecting hardware acceleration",
																	MB_OK);
	}
	else if (! generic_format && ! generic_accelerated)
	{
		// hardware - ICD
		MessageBox(NULL,"hardware - ICD","Detecting hardware acceleration",
														MB_ICONINFORMATION);
	}*/
						  ...
						  WPARAM wParam, LPARAM lParam )
{
	...
	switch ( message )
	{
		...
		case WM_SIZE:
			...
			{
				Scene.ResizeViewport(GetSystemMetrics( SM_CXSCREEN ),
		...
		case WM_KEYDOWN:
			...
			case VK_NUMPAD0:
				...
				if(fullscreen)
				{
					::SetWindowLong(hWnd, GWL_STYLE, WS_POPUP | WS_CLIPSIBLINGS | WS_VISIBLE);
				...
				{
					::SetWindowLong(hWnd, GWL_STYLE, WS_TILEDWINDOW | WS_VISIBLE);
					...
					LPSTR lpCmdLine, int iCmdShow )
{
	...
	hWnd = CreateWindowEx(NULL,
					"Name",
					"Matrix - Pasha Malinnikov demo [http://www.3d.lg.ua]",
